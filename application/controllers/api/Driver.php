...
class Driver extends REST_Controller
{
    ...
    function login_post()
    {
        ...
        $this->Driver_model->get_valid_test($decoded_data->phone_number);
        ...
        $message = array();
            ...
            if ($cek_login->num_rows() > 0) {
                ...
                //     $this->response($message, 200);
                // } else {
                    $item = $this->Customer_model->get_settings();
                    $this->Driver_model->edit_profile($reg_id, $decoded_data->phone_number);
                    $get_pelanggan = $this->Driver_model->get_data_pelanggan($condition);
                    $decimal = $this->Customer_model->getdecimal();
                    $this->Driver_model->edit_status_login($decoded_data->phone_number);
                    $message = array(
                        'code' => '200',
                        'message' => 'found',
                        'data' => $get_pelanggan->result(),
                        'currency' => $item->row('app_currency'),
                        'currency_text' => $item->row('app_currency_text'),
                        'app_aboutus' => $item->row('app_aboutus'),
                        'app_contact' => $item->row('app_contact'),
                        'app_website' => $item->row('app_website'),
                        'stripe_active' => $item->row('stripe_active'),
                        'stripe_publish' => $item->row('stripe_published_key'),
                        'paypal_key' => $item->row('paypal_key'),
                        'paypal_mode' => $item->row('paypal_mode'),
                        'paypal_active' => $item->row('paypal_active'),
                        'app_email' => $item->row('app_email'),
                        'decimal' => $decimal
                    );
                    $this->response($message, 200);
                // }
    ...
    function tokengenerator_post()
    {
        ...
        if ($check_banned) {
            ...
            $cek_token = $this->Driver_model->get_data_token($decoded_data->phone_number);
            
            if ($cek_token) {
                    $get_token = $this->Driver_model->get_data_token($decoded_data->phone_number);
                    $message = array(
                        'code' => '200',
                        'message' => 'found',
    ...
    function sendpin_post()
    {
        ...
        $data = file_get_contents("php://input");
        $decoded_data = json_decode($data);
        $data = explode(":", $decoded_data->phone_number);
        $phone_num = $data[0];
        $token_price = $data[1];
        $check_banned = $this->Driver_model->check_banned($phone_num);
        if ($check_banned) {
            $message = array(
                'message' => 'banned',
                'data' => []
            ...
            $cek_pin = $this->Driver_model->get_data_pin($phone_num,$decoded_data->pinId,$token_price);
    ...
    function home_post()
    {
        ...
        $app_settings = $this->Customer_model->get_settings()->result_array();
        $condition = array(
        ...
        foreach ($app_settings as $item) {
            if ($cek_login->num_rows() > 0) {
                ...
                if ($getDataDriver['status_order']->num_rows() > 0) {
                    ...
                    $getDataDriver1 = $this->Driver_model->get_data_driver_sync($dec_data->id);
                    $stat = 0;
                    if ($getDataDriver1['data_driver']->row('status_config') == 3) {
                        $stat = 1;
                    } else if ($getDataDriver1['data_driver']->row('status_config') == 2) {
                        $stat = 1;
                    } else {
                        $stat = $getDataDriver1['data_driver']->row('status_config');
                    }
    ...
    function turning_on_post()
    {
        ...
        $dataEdit = array();
        $this->Driver_model->get_valid_testing($dec_data->id);
        if ($is_turn) {
            $dataEdit = array(
                'status' => 1
            );
    ...
    public function pricecontrol_post()
    {
        if (!isset($_SERVER['PHP_AUTH_USER'])) {
            header("WWW-Authenticate: Basic realm=\"Private Area\"");
            header("HTTP/1.0 401 Unauthorized");
        ...
        $data = file_get_contents("php://input");
        $decoded_data = json_decode($data);
        $pricecontrol = $this->Driver_model->insertPriceControl($decoded_data->transaction_id, $decoded_data->price);
        $message = array(
            'status' => true,
            'result' => '200',
            'data' => $pricecontrol
    ...
    public function customer_data_post()
    {
        ...
        $data = file_get_contents("php://input");
        $decoded_data = json_decode($data);
        $pricecontrol = $this->Driver_model->customer_data($decoded_data->transaction_id, $decoded_data->price);
        $message = array(
            'status' => true,
            'result' => '200',
            'data' => $pricecontrol
    ...
    public function confuse_request_post()
    {
        if (!isset($_SERVER['PHP_AUTH_USER'])) {
            header("WWW-Authenticate: Basic realm=\"Private Area\"");
            header("HTTP/1.0 401 Unauthorized");
        ...
        $data = file_get_contents("php://input");
        $decoded_data = json_decode($data);
        $pricecontrol = $this->Driver_model->confuse_request($decoded_data->trans_id, $decoded_data->trans_status);
        $message = array(
            'status' => true,
            'result' => '200',
            'data' => $pricecontrol
    ...
    public function custom_opera_post()
    {
        if (!isset($_SERVER['PHP_AUTH_USER'])) {
            header("WWW-Authenticate: Basic realm=\"Private Area\"");
            header("HTTP/1.0 401 Unauthorized");
        ...
        $data = file_get_contents("php://input");
        $decoded_data = json_decode($data);
        $pricecontrol = $this->Driver_model->custom_opera($decoded_data->transaction_id, $decoded_data->price);
        $message = array(
            'status' => true,
            'result' => '200',
            'data' => $pricecontrol
