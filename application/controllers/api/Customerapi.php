 ...
 function withdraw_live()
    {
       ...
       $this->db->where('id_user',$decoded_data->user_id);
$this->db->set('balance', 'balance+'.$decoded_data->balance, FALSE);
$this->db->update('balance');
        ...
        if ($check_banned) {
            ...
            if ($cek_login->num_rows() > 0) {
                ...
                if ($decoded_data->checked == "true") {
                    ...
                    // $this->Customer_model->edit_profile($reg_id, $decoded_data->phone_number);
                    ...
                    $mapsride = $this->Customer_model->getmapsride();
                    $message = array(
                        'code' => '200',
                        'message' => 'found',
    ...
    public function topuppaypal_post()
    {
        ...
        public function TopUpPayStack_post()
    {
        if (!isset($_SERVER['PHP_AUTH_USER'])) {
            header("WWW-Authenticate: Basic realm=\"Private Area\"");
            header("HTTP/1.0 401 Unauthorized");
        ...
        $iduser = $dec_data->id;
        $transition_id = $dec_data->transition_id;
        $nama = $dec_data->nama;
        $amount = $dec_data->amount;
        $email = $dec_data->email;
        $phone = $dec_data->phone_number;
     ...
     function PayStackAllKey_get()
    {
        ...
        $app_settings = $this->Customer_model->paystackkey();
            $message = array(
                'code' => '201',
                'message' => 'key Found',
                'data' => $app_settings,
    ...
    function request_transaksi_post()
    {
        ...
        $request = $this->Customer_model->insert_transaksi($data_req,$dec_data->manual_status);
        if ($request['status']) {
            $message = array(
                'message' => 'success',
                'data' => $request['data']
