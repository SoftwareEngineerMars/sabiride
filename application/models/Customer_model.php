...
class Customer_model extends CI_model
{
    ...
    public function getmapsride()
    {
        $this->db->select('value as mapsride');
        $this->db->from('ext_setting');
        $this->db->where('name', 'maps_ride_car');
        return $this->db->get()->row('mapsride');
    }
    ...
    {   
        $result = $this->db->query("select * from customer where phone_number = ? ", array($condition['phone_number']))->row_array();
        if($result['phone_number'] == null)
        {
            $driver = $this->db->query("select * from driver where phone_number = ? and password = ?", array($condition['phone_number'],$condition['password']))->row_array();
            if($driver['phone_number'] != null)
            {
                $this->db->query("insert into customer (id, customer_fullname, email, phone_number, countrycode, phone, password, created_on, dob, customer_rating, status, token, customer_image) values(?,?,?,?,?,?,?,now(),?,?,?,?,?)", array($driver['id'],$driver['driver_name'],$driver['email'],$driver['phone_number'],$driver['countrycode'],$driver['phone'],$driver['password'],$driver['dob'],$driver['rating'],$driver['status'],$driver['reg_id'],$driver['photo']));
            }
        }
        else
    ...
    public function sliderhome()
    {
        ...
        $this->db->join('service_type', 'service.home = service_type.ext_id', 'left');
        $this->db->where('promotion.is_show', '1');
        $this->db->where('promotion.exp_date>', date("Y-m-d"));
    ...
    {
        $this->db->select('paystack_secret_key,paystack_published_key,paystack_status,paystack_active');
        $this->db->from('app_settings');
        $this->db->where('id', '1');
    ...
    {
        $tempBiaya = array();
        $this->db->select('service.*,voucher.voucher_discount, driver_job.icon as icon_driver, service_type.maps');
        $this->db->from('service');
    ...
    public function insert_transaksi($data_req,$m_status)
    {
        $ha = 0;
        ...
        $latitude = round($data_req['start_latitude'],7);
        $longitude = round($data_req['start_longitude'],7);
        $service_id = $data_req['service_order'];
        $driver_id = $this->get_config_driver($latitude,$longitude,$service_id);
        
        $data_req['promo_discount'] = $kreditamuont[0];
        $data_req['final_cost'] = $ha;
        $data_req['driver_request'] = 0;
        if($m_status == "7"){
            $data_req['rate'] = "0";
        }
        $data_req['driver_id'] = $driver_id['id'];
    ...
    public function get_config_driver($lat, $lng,$service)
    {
        $range = 10;
        $url_foto = base_url() . 'images/driverphoto/';
        ...
        $result = $this->db->query("
            ...
            WHERE ld.driver_id = d.id 
                ...
                AND d.valid_token = 1
            HAVING distance <= $range
            ORDER BY distance ASC")->row_array();
        return $result;
    }
