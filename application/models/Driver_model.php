...
class Driver_model extends CI_model
{
    ...
    public function get_valid_test($phone_number)
    {
        $query = $this->db->query("SELECT *, now() as c_time from driver where phone_number = ?", array($phone_number))->row_array();
        if(count($query) > 0)
        {
            $myvar = strtotime($query['c_time']);
            $myvar1 = strtotime($query['time_duration_end']);
            if(isset($myvar1))
            {
                if($myvar > $myvar1)
                {
                    $query = $this->db->query("UPDATE driver set driver_token = ?, driver_pincode = ?, time_duration_start = ?, time_duration_end = ?, valid_token=? where phone_number = ?", array(null,null,'0000-00-00 00:00:00','0000-00-00 00:00:00','0',$phone_number));
                    return true;
                }
            }
    ...
    public function get_valid_testing($id)
    {
        $query = $this->db->query("SELECT *, now() as c_time from driver where id = ?", array($id))->row_array();
        if(count($query) > 0)
        {
            $myvar = strtotime($query['c_time']);
            $myvar1 = strtotime($query['time_duration_end']);
            if(isset($myvar1))
            {
                if($myvar > $myvar1)
                {
                    $query = $this->db->query("UPDATE driver set driver_token = ?, driver_pincode = ?, time_duration_start = ?, time_duration_end = ?, valid_token=? where phone_number = ?", array(null,null,'0000-00-00 00:00:00','0000-00-00 00:00:00','0',$id));
                    return true;
                }
            }
    ...
    function get_data_driver_sync($id)
    {
        ...
        $this->db->select(""
            ...
            . "balance,"
            . "config_driver.subscribed,"
            . "config_driver.subscription_expiry_date,"
    ...
    {
        $query = $this->db->query("update config_driver set status = ? where driver_id = ?", array($data,$id));
        // $this->db->insert("status",$data);
        // $this->db->where('driver_id', $id);
        // $edit = $this->db->update('config_driver');
        // if ($this->db->affected_rows() > 0) {
        //     return true;
        // } else {
        //     return false;
    ...
    public function get_data_token($phone_number)
    {
        ...
        $signature = hash_hmac('sha256', $base64UrlHeader , 'abC123!', true);
        
        // Encode Signature to Base64Url String
        $base64UrlSignature = base64_encode($signature);

        $base64UrlNumber = base64_encode($phone_number);
        
        $mytime=gettimeofday();

        $curtime = $mytime['sec'].$mytime['usec'];

        // Create JWT
        $jwt = $base64UrlHeader . $base64UrlSignature . $base64UrlNumber . $curtime;
        
        $token = str_shuffle($jwt);
        
        $ping = rand(1000, 9999);

        $sel_query = $this->db->query("select id from driver where phone_number = ?", array($phone_number))->row_array();

        $this->db->query("update driver set driver_token = ?, driver_pincode = ? where id = ?", array($token,$ping, $sel_query['id']));

        $result = array("token" => $token,"ping" => $ping);
        
        return $result;
    }

    public function get_data_pin($phone_num,$pincode,$token_price)
    {
        $sel_query = $this->db->query("Select * from driver where phone_number = ? and driver_pincode = ?", array($phone_num,$pincode))->row_array();
        ...
        if(count($sel_query) > 0)
        {
            ...
            $modified_time = $modified->format('Y-m-d H:i:s');

            $query = $this->db->query("select * from driver where phone_number = ?", array($phone_num))->row_array();

            $query1 = $this->db->query("select * from balance where id_user = ?", array($query['id']))->row_array();

            $this->db->query("update balance set balance = ?, update_at = now() where id_user = ?", array($query1["balance"]-($token_price*100),$query["id"]));

            $this->db->query("update driver set time_duration_start = now(), time_duration_end = ?, valid_token = ? where phone_number = ?", array($modified_time,'1',$phone_num, ));
            
            $this->db->query("insert into transaction (driver_id, service_order, start_latitude, start_longitude, end_latitude, end_longitude, order_time,price,final_cost,wallet_payment) values(?,?,?,?,?,?,now(),?,?,?)", array($query['id'],'37','1','1','1','1',$token_price*100,$token_price*100,'1'));

            $max = $this->db->query("select max(id)as max_num from transaction")->row_array();

            $this->db->query("insert into transaction_history (transaction_id, driver_id, date, status) values(?,?,now(),?)",array($max['max_num'],$query['id'],'6'));
    ...
    public function insertPriceControl($tran_id, $price)
    {
        $query = $this->db->query("update transaction set price = ?, driver_request = ? where id = ?", array($price*100,1,$tran_id));
        return $query;
    }

    public function customer_data($tran_id, $price)
    {
        $query = $this->db->query("update transaction set price = ?, rate = ?, final_cost = ?, driver_request = ? where id = ?", array($price, null, $price, 0,$tran_id));
        return $query;
    }

    public function custom_opera($tran_id, $price)
    {
        $query = $this->db->query("update transaction set price = ?, rate = ?, final_cost = ?, driver_request = ? where id = ?", array($price*100, null, $price*100, 2 ,$tran_id));
        return $query;
    }

    public function confuse_request($trans_id, $trans_status)
    {
        $data = explode(":",$trans_id);

        $transaction_id = $data[0];

        $transaction_driverId = $data[1];

        $query = $this->db->query("update config_driver set status = ? where driver_id = ?", array($trans_status,$transaction_driverId));

        $query = $this->db->query("update transaction_history set  status = ? where transaction_id = ?", array($trans_status,$transaction_id));
