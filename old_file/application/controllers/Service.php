...
class Service extends CI_Controller
{
    ...
    public function mapstype()
    {
        ...
        $data['mapstype'] = $this->srvc->getmapstype();
        $getview['menu'] = $this->appset->getMenuAdmin();
        $getview['countnotification'] = $this->dasb->countnotif();
        $this->load->view('includes/header', $getview);
        $this->load->view('service/mapstype', $data);
        $this->load->view('includes/footer', $getview);
    }

    public function editmaps()
    {
        $maps = html_escape($this->input->post('maps', TRUE));
        $ids = html_escape($this->input->post('ext_id', TRUE));

        $arraymap = array_map(null, $ids, $maps);


        $this->form_validation->set_rules('maps', 'maps', 'trim|prep_for_form');
        ...
        if ($this->form_validation->run() == TRUE) {
            foreach ($arraymap as $map) {
                ...
                if (demo != TRUE) {
                    $success = $this->srvc->editmapstype($data, $map[0]);
                }
            }
            ...
            if (demo == TRUE) {
                $this->session->set_flashdata('demo', 'NOT ALLOWED FOR DEMO');
                redirect('service/mapstype');
            } else {
                if ($success) {
                    $this->session->set_flashdata('success', 'Maps Type Has Been Changed');
                    redirect('service/mapstype');
                } else {
                    $this->session->set_flashdata('danger', 'Error, please try again!!');
                    redirect('service/mapstype');
