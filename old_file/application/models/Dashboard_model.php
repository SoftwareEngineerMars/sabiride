...
class Dashboard_model extends CI_model
{
    ...
    {
        $this->db->select("(SELECT COUNT(dr.id) FROM driver dr WHERE status = 0) as newreg_drivercount");
        $this->db->select("(SELECT COUNT(pr.partner_id) FROM partner pr WHERE partner_status = 0) as newreg_merchantcount");

        $this->db->select("(SELECT COUNT(wl.id) FROM wallet wl 
        WHERE status = 0
        AND type = 'topup') as topupreq_notif");

        $this->db->select("(SELECT COUNT(wl.id) FROM wallet wl 
        WHERE status = 0
        AND type = 'withdraw') as withdrawreq_notif");
